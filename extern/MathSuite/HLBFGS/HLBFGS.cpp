...
void HLBFGS(int N, int M, double *x,
	...
	void EVALFUNC_H(int, double*, double*, double*, double*, HESSIAN_MATRIX&, void*),
	...
	void* user_pointer)
{
	...
	do
	{
		...
		{
			EVALFUNC_H(N, x, INFO[2] == 0 ? 0 : prev_x, &f, g, m_hessian, user_pointer);
			HLBFGS_BUILD_HESSIAN_INFO(m_hessian, INFO);
			printf("-----------------------3\n");
		}
		else if (INFO[2] == 0)
		{
			EVALFUNC(N, x, 0, &f, g, user_pointer);
			INFO[1]++;
		}
...
#pragma omp parallel for private(i)
		...
		do
		{
			...
			if (blinesearch)
			{
				EVALFUNC(N, x, prev_x, &f, g, user_pointer);
				INFO[1]++;
			}
		...
		gnorm = HLBFGS_DNRM2(N, g);
		INFO[2]++;
		NEWITERATION(INFO[2], INFO[1], x, &f, g, &gnorm, user_pointer);
